...
func retrievePlayer(ctx context.Context, tenantDB dbOrTx, id string, tenantID int64) (*PlayerRow, error) {
	...
	cacheKey := getTenantPlayerCacheKey(tenantID, id)
	cachedPlayer, found := tenantPlayerCache.Get(cacheKey)
	if found {
		p = cachedPlayer.(PlayerRow)
	} else {
...
func calcBillingReportByCompetition(ctx context.Context, tenantDB dbOrTx, tenantID int64, competitionID string) (*BillingReport, error) {
	...
	if err := adminDB.SelectContext(
		...
		return nil, fmt.Errorf("error Select visit_history2: tenantID=%d, competitionID=%s, %w", tenantID, comp.ID, err)
	}
...
func initializeHandler(c echo.Context) error {
	...
	tenantDBConnMap.Range(func(k any, v any) bool {
		tenantDBConnMap.Delete(k)
		db, ok := v.(*sqlx.DB)
		if !ok {
			return false
		}
